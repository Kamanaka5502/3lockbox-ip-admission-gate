...
app = FastAPI(
    ...
    version="1.1.0",
    description="Deterministic IP admission control using IP2Location.io evidence, replayable receipts, and a tamper-evident audit ledger.",
)
...
@app.get("/health")
def health():
    return {"status": "ok", "service": "3LOCKBOX IP Admission Gate", "policy_version": POLICY_VERSION, "evidence_provider": "IP2Location.io", "ledger": ledger.verify()}
...
async def authorize(request: AdmissionRequest):
    ...
    return Response(status_code=status_code, headers={
...
def replay(receipt: AdmissionReceipt):
    if receipt.policy_version != POLICY_VERSION:
        raise HTTPException(status_code=409, detail=f"Policy version mismatch: receipt={receipt.policy_version}, runtime={POLICY_VERSION}")
    decision, rules = evaluate(receipt.input, receipt.evidence)
    recalculated = compute_decision_hash(POLICY_VERSION, decision, receipt.input, receipt.evidence, rules)
    return {
